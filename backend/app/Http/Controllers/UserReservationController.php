...
class UserReservationController extends Controller
{
    ...
    {   
        ...
        if (!$user1) {
            ...
            $apiError->setStatus("404");
            $apiError->setCode("USER_NOT_FOUND");
            $apiError->setMessage("User with id not found");
    
            return response()->json($apiError,404);
        }

        $data = UserReservation::select('user_reservations.*', 'user_reservations.id as user_reservations_id',
         'reservations.*', 'reservations.id as reservations_id', 'rooms.*', 'rooms.id as rooms_id',
         'hotels.*', 'hotels.id as hotels_id')
            ->join('reservations', 'user_reservations.reservation_id', '=', 'reservations.id')
            ->join('rooms', 'reservations.room_id', '=', 'rooms.id')
            ->join('hotels', 'rooms.hotel_id', '=', 'hotels.id')
            ->where('user_reservations.user_id', '=', $id)
            ->simplePaginate($req->has('limit')?$req->limit:15);
